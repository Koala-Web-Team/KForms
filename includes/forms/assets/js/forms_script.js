	...
	function koalaMap() {
		...
		map = new google.maps.Map(document.getElementById('map'), {
			center: {lat: 40.7413549, lng: -73.9980244}, // TODO: Make it Dynamic
			zoom: 10, // TODO: Make it Dynamic
			styles: styles,
			mapTypeControl: true,
		});
		//select map style
		...
		for ( let i = 0; i < locations.length; i++ ) {
			...
			//show lat , lng in hidden input
			marker.addListener('dblclick', ( mapsMouseEvent ) => {
				const show_marker_data = document.getElementById("show_marker_data");
				show_marker_data.value = JSON.stringify(mapsMouseEvent.latLng.toJSON(), null, 2);
			});
		}
		...
		map.addListener('click', ( mapsMouseEvent ) => {
			...
			show_data.value = JSON.stringify(mapsMouseEvent.latLng.toJSON(), null, 2);
		});
	}
				...
				if ( element.status === "OK" ) {
					...
					if ( duration <= maxDuration ) {
						...
						const infowindow = new google.maps.InfoWindow({
							content: durationText + ' away, ' + distanceText + '<div>' +
								'<input type=\"button\" value=\"View Route\" onclick =' +
